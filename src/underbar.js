(function() {
  ...
  // Returns whatever value is passed as the argument. This function doesn't seem very useful, but remember it--if a function needs to provide an iterator when the user does not pass one in, this will be handy.
   ...
   * ===========
   *
   * In this section, we'll have a look at functions that operate on collections of values; in JavaScript, a 'collection' is something that can contain a number of values--either an array or an object.
   *
   * IMPORTANT NOTE!
   * ===========
   *
   * The .first function is implemented for you, to help guide you toward success in your work on the following functions. Whenever you see a portion of the assignment pre-completed, be sure to read and understand it fully before you proceed. Skipping this step will lead to considerably more difficulty implementing the sections you are responsible for.
   */
  ...
  _.indexOf = function(array, target) {
    // TIP: Here's an example of a function that needs to iterate, which we've implemented for you. Instead of using a standard `for` loop, though, it uses the iteration helper `each`, which you will need to write.
    var result = -1;
  ...
  // map() is a useful primitive iteration function that works a lot like each(), but in addition to running the operation on all the members, it also maintains an array of results.

  /*
   TIP: map is really handy when you want to transform an array of values into a new array of values. _.pluck() is solved for you as an example of this.
  */
  // Takes an array of objects and returns and array of the values of a certain property in it. E.g. take an array of people and return an array of just their ages
  ...
  _.defaults = function(obj) {
    _.each(arguments, function(argumentsObj) {
      for (let key in argumentsObj) {
        if (obj[key] === undefined) {
          obj[key] = argumentsObj[key];
        }
      }
    });
    return obj;
  ...
  _.once = function(func) {
    // TIP: These variables are stored in a "closure scope" (worth researching), so that they'll remain available to the newly-generated function every time it's called.
    let alreadyCalled = false;
    let result;
    // TIP: We'll return a new function that delegates to the old one, but only if it hasn't been called before.
    return function() {
      if (!alreadyCalled) {
  ...
  // Memorize an expensive function's results by storing them. You may assume that the function only takes primitives as arguments. memoize could be renamed to oncePerUniqueArgumentList; memoize does the same thing as once, but based on many sets of unique arguments.
  ...
  //
  // TIP: This function's test suite will ask that you not modify the original input array. For a tip on how to make a copy of an array, see:
